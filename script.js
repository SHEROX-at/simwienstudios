...
function init(){
  if(userData.name){ showHome(); } else { renderStep(); }
}
function showHome(){
...
function startNewCalculation(){
  currentStep=0; serviceIndex=0; renderStep();
...
function askService(){
  ...
  if(s.date){ c.innerHTML=`<h2>${s.label}</h2><p>${userData.name}, wann wurde das zuletzt gewechselt?</p><input type='date' id='val' value='${userData[s.key]||''}'><button onclick='saveService()'>Weiter</button>`; }
    ...
    c.innerHTML=`<h2>${s.label}</h2><p>${userData.name}, bei wie viel km war der letzte Wechsel?</p><input type='number' id='val' value='${userData[s.key]||''}'><p>Intervall:</p><select id='int'>${opts}</select><button onclick='saveService(true)'>Weiter</button>`;
  } else {
...
function saveService(hasInt){
  const s=services[serviceIndex];
  if(s.date){ userData[s.key]=document.getElementById('val').value; } else { userData[s.key]=parseInt(document.getElementById('val').value)||0; }
  if(hasInt){ userData['beltInterval']=parseInt(document.getElementById('int').value); }
  serviceIndex++; askService();
}
function finishCalc(){
  userData.date=new Date().toLocaleDateString(); history.push({...userData}); localStorage.setItem('wartungsDaten',JSON.stringify(userData)); localStorage.setItem('wartungsHistory',JSON.stringify(history)); showDashboard();
}
function showDashboard(){
  document.getElementById('stepContainer').style.display='none'; document.getElementById('homePage').style.display='none'; document.getElementById('dashboard').style.display='block';
  const km=userData.km; const nextBrake=new Date(userData.brake); nextBrake.setFullYear(nextBrake.getFullYear()+2);
  const pb=document.getElementById('progressBars'); pb.innerHTML='';
  const items=[
    {label:'Ölwechsel',remain:15000-(km-userData.oil)},
    {label:'Kraftstofffilter',remain:30000-(km-userData.fuel)},
    {label:'Bremsflüssigkeit',remain:`am ${nextBrake.toLocaleDateString()}`},
    {label:'Luftfilter',remain:20000-(km-userData.air)},
    {label:'Servolenkungsöl',remain:50000-(km-userData.steering)},
    {label:'Zahnriemen',remain:userData.beltInterval-(km-userData.belt)}
  ];
  items.forEach(it=>{
    const cont=document.createElement('div'); cont.className='progress-container';
    cont.innerHTML=`<div class='progress-label'>${it.label}: ${typeof it.remain==='string'?it.remain:it.remain+' km'}</div>`;
    if(typeof it.remain==='number'){ const perc=Math.max(0,Math.min(100,(it.remain/ (it.label==='Ölwechsel'?15000:it.label==='Kraftstofffilter'?30000:it.label==='Luftfilter'?20000:it.label==='Servolenkungsöl'?50000:userData.beltInterval))*100)); cont.innerHTML+=`<div class='progress-bar'><div class='progress-fill' style='width:${perc}%'></div></div>`; }
    pb.appendChild(cont);
  });
}
function goHome(){
  document.getElementById('dashboard').style.display='none'; showHome();
}
init();
